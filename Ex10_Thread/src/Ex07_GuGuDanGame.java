...
class TimeOut extends Thread{
    ...
    public void run() {
        ...
        Ex07_GuGuDanGame.inputcheck = true; //시간 종료
...
class WordGame extends Thread{
    ...
    public void run() {
        ...
        Ex07_GuGuDanGame.totalQuizCount = gugudan.size();
        ...
        for(int i = 0 ; i < keylist.length ; i++) {
            ...
            if(Ex07_GuGuDanGame.inputcheck) break;         
            if(gugudan.get(keylist[i])==answer) {
                System.out.println("당신은 정답");
                Ex07_GuGuDanGame.QuizUserInfo.add(new QuizInfo((String)keylist[i], answer, "O"));
            }else {
                System.out.println("당신은 오답");
                Ex07_GuGuDanGame.QuizUserInfo.add(new QuizInfo((String)keylist[i], answer, "X"));
            }
        }
        //System.exit(0);
    }
